...
def convolutional_block(i_tensor, i_features, o_features, window_size=3, scope=None):
    with tf.variable_scope(scope):
        kernel = self.var(input_shape=[window_size, i_features, o_features])
        x = tf.nn.conv1d(i_tensor, kernel, stride=1, padding="SAME")
        conv = tf.nn.relu(x)
        return conv

def max_pool_layer(i_tensor, window_size, strides, scope=None):
    _ = tf.reshape(i_tensor, [-1, 1, i_tensor.shape[1].value, i_tensor.shape[2].value])
    _ = tf.nn.max_pool(_, [1, 1, window_size, 1], [1, 1, strides, 1], 'SAME')
    _ = tf.reshape(_, [-1, _.shape[2].value, _.shape[3].value])
    return _


def max_pooled_convolutional_layer(i_tensor, i_features=1, o_features=1, conv_window_size=2,
                                   conv_strides=2, pool_window_size=2, pool_strides=2):
    _ = convolutional_block(i_tensor=i_tensor, i_features=i_features, o_features=o_features, window_size=conv_window_size)
    return max_pool_layer(i_tensor=_, window_size=pool_window_size, strides=pool_strides)
...
def inference():
    ...
    _ = max_pooled_convolutional_layer(input_layer, byte_encoder_depth, convolutional_features, 7, 1, 3, 2)
    _ = max_pooled_convolutional_layer(_, convolutional_features, convolutional_features, 7, 1, 3, 2)
    _ = convolutional_block(i_tensor=_, i_features=convolutional_features, o_features=convolutional_features, window_size=3)
    _ = convolutional_block(i_tensor=_, i_features=convolutional_features, o_features=convolutional_features, window_size=3)
    encoding_layer = tf.nn.rnn_cell.LSTMCell(self.encoder_internal_size)
    Yr, H = tf.nn.dynamic_rnn(encoding_layer, _, dtype=tf.float32)
    encoded_text = Yr[:, -1, :]
