...
def generate_batches(min_id=0, max_id=None, batch_size=10, epochs=None):
    gen = db_connection.batches('trinary_sentiment_dataset', 'id', ['text'], batch_size=batch_size, epochs=epochs, min_id=min_id, max_id=max_id)
    sentiment_map = {0: 0, 1: 1, -1: 2}
    for b in iter(gen):
        batch_x = []
        batch_y = []
        for row in b:
            bytes_ = [ord(x) for x in row['text'] if 0 < ord(x) < 256]
